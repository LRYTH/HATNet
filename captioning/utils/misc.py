...
def save_checkpoint(opt, model, infos, optimizer, histories=None, append=''):
    ...
    # 同时保存超参数为 YAML 文件
    _save_hparams_to_yaml(opt, append)

def _save_hparams_to_yaml(opt, append=''):
    """将训练超参数保存为可读的 YAML 文件"""
    # 定义需要保存的参数（排除内部/临时属性）
    exclude_keys = {
        'vocab', 'cfg', 'set_cfgs', 'checkpoint_path', 'start_from',
        'input_json', 'input_label_h5', 'input_att_dir', 'input_fc_dir',
        'input_box_dir', 'cached_tokens',
    }
    # 数据类型白名单（只保存基本类型）
    ...
    for key in sorted(vars(opt)):
        if key in exclude_keys:
            continue
        val = getattr(opt, key)
        if isinstance(val, allowed_types) and not key.startswith('_'):
            hparams[key] = val

    yaml_path = os.path.join(opt.checkpoint_path, 'hparams%s.yaml' % append)
    try:
        import yaml
        with open(yaml_path, 'w', encoding='utf-8') as f:
            yaml.dump(hparams, f, default_flow_style=False, sort_keys=False, allow_unicode=True)
        print("hparams saved to {}".format(yaml_path))
    except Exception as e:
        print("Warning: failed to save hparams yaml: {}".format(e))
